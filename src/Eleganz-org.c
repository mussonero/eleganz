...
#define DEV_USB_FMT1 "/dev_usb000/"
...
#define USB_PICS_DIRECTORY_FMT DEV_USB_FMT1 "PICS/"
#undef KEYS_PATH
#ifdef __lv2ppu__
...
_game_selected (Item *item)
{
  ...
  if (strcmp (game->path + strlen (game->path) - 4, ".RUN") == 0) {
    ...
    cobra_manager_eject(&self->cobra);
    cobra_manager_free (&self->cobra);
    sysMemContainerDestroy(&self);
    sysProcessExit(&self);
...
_games_new (Eleganz *self, const char *path)
{
  ...
  if (strcmp (path, GAMES_DIRECTORY) == 0) {
    ...
    game->title = _strdup_printf ("COBRA ODE v%d.%d", self->cobra.info.major, self->cobra.info.minor);
    ...
    for (i = 0; i < self->cobra.num_iso; i++) {
      ...
      if (self->cobra.iso[i].game_id)
    	game->picture = _strdup_printf ("%s/%s.PNG", USB_PICS_DIRECTORY_FMT, self->cobra.iso[i].game_id);
      else
      game->picture = NULL;
      games = eina_list_append (games, game);
    }
